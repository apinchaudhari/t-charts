...
use App\Models\Superadmin\Auth\Admin;
...
class Admin extends Controller
{
    ...
    public function index()
    {
        ...
        return $this->response('superadmin.auth.admin.index', compact('admin'));
    }
    ...
    public function create()
    {
        ...
        return view('superadmin.auth.admin.create', compact('roles', 'companies', 'landing_pages'));
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($response['success']) {
            $response['redirect'] = route('superadmin.admin.index');

            $message = trans('messages.success.added', ['type' => trans_choice('general.users', 1)]);

            flash($message)->success();
        } else {
            $response['redirect'] = route('superadmin.admin.create');
    ...
    public function edit(User $user)
    {
        ...
        return view('auth.superadmin.admin.edit', compact('user', 'companies', 'roles', 'landing_pages'));
    }
    ...
    public function update(User $user, Request $request)
    {
        ...
        if ($response['success']) {
            $response['redirect'] = user()->can('read-auth-users') ? route('superadmin.admin.index') : route('superadmin.admin.edit', $user->id);

            $message = trans('messages.success.updated', ['type' => $user->name]);

            flash($message)->success();
        } else {
            $response['redirect'] = route('superadmin.admin.edit', $user->id);
